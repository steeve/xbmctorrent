...
def get_season_list_item(meta, season):
    ...
    #     if banner["bannertype"] == "season" and int(banner["season"]) == season:
    #         return {
    #             "label": "Season %d" % season,
    #             "icon": m(""),
...
def get_episode_list_item(show_meta, episode):
    m = lambda x: meta.get(x) or ""
    m_list = lambda x: meta.get(x) and ", ".join(meta[x]) or ""
    for meta in show_meta["episode"]:
        if int(meta["seasonnumber"]) == season and int(meta["episodenumber"]) == episodenumber:
            return {
                "label": meta["episodename"],
                "icon": m("filename"),
                "thumbnail": m("filename"),
                "info": {
                    "count": meta["id"],
