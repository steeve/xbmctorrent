...
TORRENT2HTTP = xbmcaddon.Addon("script.module.torrent2http")
BIN_PATH = os.path.join(TORRENT2HTTP.getAddonInfo("path"), "bin", "%(os)s_%(arch)s" % PLATFORM)
...
def start(**kwargs):
    ...
    def proc_close():
        if not proc.poll():
            url_get("http://%s/shutdown" % bind_address)
