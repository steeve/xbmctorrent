...
def parse(data, content_type=None):
    ...
    for node in root.getiterator("item"):
        item = {
            ...
            "seeds": _text(node, ".//{%(torrent)s}seeds" % NSMAP) or _text(node, "numSeeders"),
            "peers": _text(node, ".//{%(torrent)s}peers" % NSMAP) or _text(node, "numLeechers"),
