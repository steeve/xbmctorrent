...
from xbmctorrent.utils import first, terminating
...
def eztv_shows_by_letter(letter):
    ...
    if not len(shows_cache.keys()):
        response = session.get("http://eztv.it/showlist/")
    ...
    def _eztv_shows_by_letter(letter):
        with closing(xbmcgui.DialogProgress()) as dialog:
            ...
            dialog.update(percent=0, line1="Fetching serie information...")
...
def eztv_get_show_seasons(show_id):
    ...
    with terminating(ThreadPool(2)) as pool:
        def _eztv_get_show():
            ...
            response = session.get("http://eztv.it/shows/%s/" % show_id)
