...
BASE_URL = "http://%s/" % plugin.get_setting("domain_yify")
HEADERS = {
    "Referer": BASE_URL,
}
...
def yify_genres():
    for genre in GENRES:
        yield {
            "label": genre,
            "path": plugin.url_for("yify_genre", genre=genre, sort="alphabet", order="asc", set=1, limit=MOVIES_PER_PAGE),
        }
...
def yify_search():
    ...
    if query:
        plugin.redirect(plugin.url_for("yify_search_query", keywords=query, set=1, limit=MOVIES_PER_PAGE))
