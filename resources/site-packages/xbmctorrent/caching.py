...
def shelf(filename, ttl=0):
    import shelve
    filename = os.path.join(plugin.storage_path, filename)
...
def cached_route(*args, **kwargs):
    ...
    def cached(fn):
        ...
        def _fn(*a, **kwds):
            ...
            filename = os.path.join(plugin.storage_path, basename)
            with shelf(filename, ttl=kwargs.get("ttl") or 0) as result:
